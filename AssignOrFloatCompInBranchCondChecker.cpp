...
//checks if right operand of comma operator in condition statement is assignment, W-parenthesis rule already checks if ONLY expression in condition statement is assignment
...
bool ento::shouldRegisterAssignOrFloatCompInBranchCondChecker(const CheckerManager &mgr) {
  return true;
}
